  ...
  <div>
    <Card :bordered="false" style="margin-bottom:20px;">
      ...
      <Form :model="formItem" :label-width="80" :inline="true" style="margin-top: 20px">
        ...
        <FormItem label="计费方式:">
          <Select v-model="formItem.chargeType" style="width:200px" @on-change='searchAllData(true)'>
            <Option :value="-1">全部</Option>
            <Option v-for="(value, key) in chargedTypes" :value="key" :key="key">{{ value }}</Option>
          </Select>
        </FormItem>
        <FormItem :label-width="30">
          ...
          <Button
            ...
            v-if="showAccess('数据统计_CPS/CPA计费表_导出')"
          >导出</Button>
          <Button
            type="primary"
            style="margin-left:10px"
            @click="addCPSCPAList = true"
            v-if="showAccess('数据统计_CPS/CPA计费表_新增')"
          >新增</Button>
          <Button
            type="primary"
            style="margin-left: 10px"
            @click="openImportModal"
            v-if="showAccess('数据统计_CPS/CPA计费表_批量导入')"
          >批量导入</Button>
        </FormItem>
      </Form>
    ...
    <Card :bordered="false">
      <div slot="title">
        <div style="display:flex;justify-content:space-between;">
          <span style="font-size:20px;line-height:32px;font-weight:bold;">金额明细</span>
          <Button type="primary" style="margin-left:10px" @click="openIncomeList" v-if="showAccess('数据统计_CPS/CPA计费表_生成应收单')"">生成应收单</Button>
        </div>
      </div>
      <div class="layout-table">
        ...
        >
          <template slot-scope="{ row }" slot="estimatedAmount">
            ...
            >
              ...
              <Icon
                ...
                size="20"
                v-if="showAccess('数据统计_CPS/CPA计费表_编辑预估金额') && row.collectStatus !== 1"
              />
            </div>
          </template>
          <template slot-scope="{row, index}" slot="content">
          ...
          <template slot-scope="{ row }" slot="baseInfoUpdateTime">
            <span>{{row.baseInfoUpdateTime | formatDate}}</span>
          </template>
          <template slot-scope="{ row }" slot="statisticsMonth">
            <span>{{row.statisticsMonth}}</span>
          </template>
          <template slot-scope="{ row }" slot="chargedType">
            <span>{{chargedTypes[row.chargedType]}}</span>
          </template>
          <template slot-scope="{ row }" slot="industry">
            <span>{{row.industry && industry.length ? industry.filter(ind => ind.key == row.industry)[0].value : ''}}</span>
          </template>
          <template slot-scope="{row, index}" slot="action">
            <Button
              ...
              size="small"
              v-if="showAccess('数据统计_CPS/CPA计费表_编辑')"
            >编辑</Button>
            <Button
              style="margin-left: 10px;"
              type="error"
              ...
              size="small"
              v-if="showAccess('数据统计_CPS/CPA计费表_删除') && row.collectStatus !== 1 "
            >删除</Button>
          </template>
        </Table>
        <div style="display:flex;justify-content:flex-end;margin-top:10px;">
    ...
    <Modal
      :title="addType == 'update' ? '编辑CPS/CPA计费信息' : '新增CPS/CPA计费信息'"
    ...
    >
      ...
      >
        ...
        <FormItem label="版图:" prop="industry">
        ...
        <FormItem label="推广内容：" prop="content">
          <Input v-model="cpscpaForm.content" placeholder="请填写推广内容" clearable></Input>
        </FormItem>
        <FormItem label="计费方式：" prop="chargedType" style="width: 100%">
          <RadioGroup v-model="cpscpaForm.chargedType">
            <Radio v-for="(value, key) in chargedTypes" :label="key" :key="key">{{value}}</Radio>
          </RadioGroup>
        </FormItem>
        <FormItem label="结算金额：" prop="realTotal">
            <Input v-model="cpscpaForm.realTotal" placeholder="请填写结算金额" clearable :disabled="!!cpscpaForm.collectionTotal"><span slot="append">元</span></Input>
          </FormItem>
          <FormItem label="预估金额：" prop="estimatedAmount">
            <Input v-model="cpscpaForm.estimatedAmount" placeholder="请填写预估金额" clearable :disabled="cpscpaForm.collectStatus===1&&addType=='update'"><span slot="append">元</span></Input>
          </FormItem>
      </Form>
      <div slot="footer">
    ...
    >
      <div class="importContent">
        <Upload
          ref="uploadBatch"
          action="/finance/cpscpanew//import"
          accept=".xls, .xlsx"
          :headers="headers"
        ...
        <a :href="onloadUrl + 'CPS_CPA计费表导入模板.xlsx'" download="CPS_CPA计费表导入模板.xlsx">
          <Button
    ...
    <Modal title="应收单列表" v-model="incomeListModal" :mask-closable="false" width="1200">
      <div>
        <div>
          <Form :model="incomeFormItem" :label-width="80" :inline="true" style="margin-top:20px;">
            <FormItem label="客户名称:">
              <Input
          ...
          >
            <template slot-scope="{row}" slot="collectStatus">
              <span v-if="row.collectStatus" style="color: #007fff">已生成应收</span>
              <span v-else style="color: #e04a57">待生成应收</span>
            </template>
            <template slot-scope="{row}" slot="aeNames">
              <span>{{row.aeNames.length ? row.aeNames.join(',') : ''}}</span>
            </template>
            <template slot-scope="{row}" slot="action">
              <Button type="dashed" size="small" v-if="row.collectStatus">已生成应收</Button>
              <Button type="primary" size="small" v-if="row.collectStatus && showAccess('数据统计_CPS/CPA计费表_重新生成应收')" @click="createIncome(row, 'again')" style="margin-left: 8px;">重新生成应收</Button>
              <Button type="primary" size="small" v-if="!row.collectStatus" @click="createIncome(row)">生成应收</Button>
            </template>
          </Table>
          <div style="display:flex;justify-content:flex-end;margin-top:10px;">
            <Page
  ...
  methods: {
    getAdTerritoryList() {
      axios
          ...
          .then(res => {
            if (res.data.code === 0) {
              this.industry.push(...res.data.data);
            } else {
    ...
    searchData(isfirst, type) {
      ...
      let postData = {
        statisticsMonthEnd: this.formItem.yyyymm[1] ? moment(this.formItem.yyyymm[1]).format("YYYYMM") : '',
        statisticsMonthStart: this.formItem.yyyymm[0] ? moment(this.formItem.yyyymm[0]).format("YYYYMM") : '',
      ...
      if (type == "Detail" && this.orderData.orderType && this.orderData.orderType !== 'normal') {
        postData.orderColumn = this.orderData.orderColumn
        postData.orderType = this.orderData.orderType == 'desc' ? 0 : 1 
      }
      let _this = this;
      axios
        .request({
          url:
            type == "Total"
              ? "/finance/cpscpanew/statisticByCustomerAndProduct"
              : "/finance/cpscpanew/find",
          method: "post",
          data: postData
        })
        .then(d => {
          if (d.data.code == 0) {
            ...
            // _this['rowData' + type] = [{estimatedAmount: '1212',id:1},{estimatedAmount: '1231231',id:2}]
    ...
    sortColumnHandler({column, key, order}) {
      ...
      this.searchDetailData(true)
    },
    editCPSCPAList(row, type) {
      this.addCPSCPAList = true
      axios.request({
        url: '/finance/cpscpanew/getById',
        method: 'post',
        data: {
          id: row.id
        }
    ...
    checkResult() {
      axios
        ...
        .then(res => {
          ...
          link.setAttribute("download", "批量导入PCS/CPA计费表结果.xlsx");
    ...
    addSubmit(name) {
      let _this = this
        this.$refs[name].validate(valid => {
            if (valid) {
                let param = {}
                let url = this.addType == 'update' ? '/finance/cpscpanew/update' : '/finance/cpscpanew/add'
                let postData = {..._this.cpscpaForm, statisticsMonth: moment(this.cpscpaForm.statisticsMonth).format("YYYYMM")}
                axios
                  .request({
                    url,
                    method: "post",
    ...
    searchDataIncome(flag) {
      ...
      let postData = {
        statisticsMonthEnd: this.incomeFormItem.time[1]
          ? moment(this.incomeFormItem.time[1]).format("YYYYMM")
          : "",
        statisticsMonthStart: this.incomeFormItem.time[0]
      ...
      axios
        .request({
          url: "/finance/cpscpanew/queryStatementStatisticsWithNoReturnStatus",
          method: "post",
          data: postData
        })
        .then(d => {
    ...
    batchCreateAxios(ids, type) {
      ...
      axios
        .request({
          url: type === 'again' ? '/finance/cpscpanew/reCreateCpscpaCollectionMoneyBill' : "/finance/cpscpanew/createCpscpaCollectionMomeyBill",
          method: "post",
          data: postData
        })
        .then(d => {
