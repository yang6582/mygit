<template>
  <div>
    ...
    <Card :bordered="false">
      <p slot="title" style="font-size:20px;">金额明细</p>
          ...
          <template slot-scope="{ row }" slot="menuUrlDes">
            <Tooltip placement="top" transfer>
              ...
              <a :href="row.menuUrlDes" target="_blank">
                <Button size="small" style="color:#59afed" v-if="row.date != '总计'">查看</Button>
              </a>
            </Tooltip>
          </template>
        </Table>
...
export default {
  ...
  methods: {
    ...
    searchData(isfirst) {
      ...
      axios
        .request({
          url: "/finance/queryCPCConsumptionStatistic",
          method: "post",
          data: postData
        })
        .then(d => {
    ...
    downloadReport() {
      ...
      axios
        .request({
          url: "/finance/exportCPCConsumptionStatistic",
          method: "post",
          data: postData,
          responseType: "blob"
        })
