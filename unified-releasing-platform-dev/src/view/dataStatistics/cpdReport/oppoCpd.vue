<template>
  <div class="OPPO-CPD">
    <Form ref="form-inline" :model="formItem" inline :label-width="80">
      ...
      <FormItem :label-width="10">
        <Button type="primary" @click="dataExport" v-if="showAccess('数据统计_CPD明细数据报表_OPPO-导出')">导出</Button>
      </FormItem>
    </Form>
    <Table border
        :columns="columns"
  ...
  export default {
    ...
    methods: {
      ...
      dataExport(){
        ...
        axios
          .request({
            url: "ad/data/statistics/cpd/export/oppo/detail",
            method: "post",
            data: postData,
            responseType: 'blob'
          }).then(res => {
            if (res.status == 200) {
                ...
                const fileName = "OPPO-CPD报表导出" + ".xls";
      ...
      searchData(isfirst) {
        ...
        axios
          .request({
            url: "ad/data/statistics/cpd/oppo/detail",
            method: "post",
            data: postData
          })
          .then(d => {
