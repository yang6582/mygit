  ...
  <div class="expendTableClass">
    <Form :model="formItem" :label-width="80" :inline="true" style="margin-top:20px;">
      ...
      <FormItem :label-width="30">
        ...
        <Button type="primary" @click="exportData" style="margin-left: 10px">导出</Button>
      </FormItem>
    </Form>
    <div>
      <Table
      ...
      >
        ...
        <template slot-scope="{ row }" slot="operator">
          <Button type="success" size="small" @click="openMaintainReback(row)" v-if="showAccess('应收管理_非C应收管理明细表_回款维护')">回款维护</Button>
          <Button type="primary" size="small" style="margin-left: 10px" @click="openReceipt(row)">回款</Button>
        </template>
      </Table>
      <Page
...
export default {
  ...
  methods: {
    ...
    searchData(flag) {
      ...
      let postData = {
        statisticsMonthStart: this.formItem.time[0].split("-").join(""),
        statisticsMonthEnd: this.formItem.time[1].split("-").join(""),
        // statisticsMonthEnd:'',
      ...
      axios
        .request({
          url: "/finance/cpscpanew/queryStatementStatistics",
          method: "post",
          data: postData
        })
        .then(res => {
    ...
    exportData() {
      ...
      axios
        .request({
          url: "/finance/cpscpanew/exportStatementStatistics",
          method: "post",
          data: postData,
          responseType: "blob"
        })
 ...
 // this.$refs.uploadVoucher.clearFiles();
      ...
      this.maintainModalData.remainDebt = row.realTotal - row.collectionTotal;
      this.visiableMaintainModal = true
    },
    openReceipt(row) {
      this.maintainModal = true;
      // this.formItem2.companyName = "";
    ...
    closeMaintainModalHandler(data) {
      if (data) {
        let postData = {
          ...
          axios
            .request({
              url: "/finance/addAdCpsCpaConsumeBillReturnMoneyRecord",
              method: "post",
              data: postData
            })
            .then(res => {
    ...
    submitMaintain() {
      ...
      if (arr.length) {
        ...
        // data.append("statementNo", this.formItem2.statementNo);
        // data.append("collectionId", arr[0].id);
        // data.append("collectionAmount", arr[0].collectionAmount)
        let data = {
          consumeMonthBillId: this.formItem2.consumeMonthBillId,
          collectionInfos: arr
        }
        axios
          .request({
            url: "/finance/cpscpanew/batchSelectCollectionByStatement",
            method: "post",
            data
          })
          .then(res => {
