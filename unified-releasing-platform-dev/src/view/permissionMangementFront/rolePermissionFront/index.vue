<template>
  <div class="rolePermission">
    ...
    <Modal
      ...
      :mask-closable="false"
      :width="addModalType == 'add' ? '70' :editType == 'top' ?'550':'70'"
      :styles="{top: '30px'}"
...
export default {
  ...
  methods: {
    confirmEdit() {
      if (this.addModalType == "add") {
        ...
        if (this.editType == "bottom") {
          ...
          axios
            .request({
              url: "/integrate/auth/role/resource/permission/set",
              method: "post",
              data: postData
            })
            .then(res => {
          ...
          this.$refs["editForm"].validate(valid => {
            if (valid) {
              ...
              axios
                .request({
                  url: "/integrate/auth/role/update",
                  method: "post",
                  data: postData
                })
                .then(res => {
    ...
    getRowPermission(row) {
      axios
        .request({
          url: "/integrate/auth/role/resource/permission/get/" + row.roleId,
          method: "get"
        })
        .then(res => {
          if (res.data.code === 0) {
    ...
    delRoleOk(row) {
      axios
        .request({
          url: "/integrate/auth/role/delete/" + row.roleId,
          method: "get"
        })
        .then(res => {
          if (res.data.code === 0) {
    ...
    getAllPermission() {
      axios
        .request({
          url: "/integrate/auth/resource/all/1",
          method: "get"
        })
        .then(res => {
          if (res.data.code === 0) {
