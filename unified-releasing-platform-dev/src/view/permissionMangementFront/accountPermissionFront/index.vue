    ...
    <Modal v-model="editModal" :title="editType == 'add' ? '新增': '编辑'" :mask-closable="false">
      <Form ref="editForm" :model="editForm" :rules="addRuleValidate" :label-width="110">
        ...
        <FormItem label="密码:" prop="userPwd">
          <Input v-model.trim="editForm.userPwd" style="width:300px;" clearable>
            <Icon type="ios-lock-outline" slot="prepend"></Icon>
          </Input>
        </FormItem>
        <FormItem label="联系人:" prop="nickName">
          <Input v-model.trim="editForm.nickName" style="width:300px;" :maxlength="20" clearable>
            <Icon type="ios-person-outline" slot="prepend"></Icon>
          </Input>
        </FormItem>
        <FormItem label="邮箱:" prop="userEmail">
          <Input v-model.trim="editForm.userEmail" style="width:300px;" clearable>
            <Icon type="ios-mail-outline" slot="prepend" />
          </Input>
        </FormItem>
        <FormItem label="电话号码:" prop="userPhone">
          <Input v-model.trim="editForm.userPhone" style="width:300px;" clearable>
...
export default {
  ...
  data() {
    ...
    const validatePassword = (rule, value, callback) => {
      ...
      if (value.trim() == "") {
        ...
        callback(new Error("密码由8-18位数字或字母组成"));
      }
    };
    const validatePhone = (rule, value, callback) => {
      let reg = /^1[3456789]\d{9}$/;
      if (value.trim() == "" || (value && reg.test(value))) {
        callback();
      } else {
  ...
  methods: {
    ...
    confirmEdit() {
      this.$refs["editForm"].validate(valid => {
        if (valid) {
          ...
          let url =
            this.editType == "add"
              ? "/integrate/auth/user/add"
              : "/integrate/auth/user/update";
          this.editType != "add"
    ...
    delRoleOk(row) {
      axios
        .request({
          url: "/integrate/auth/user/delete/" + row.userId,
          method: "get"
        })
        .then(res => {
          if (res.data.code === 0) {
    ...
    getRoleNameList() {
      axios
        .request({
          url: "/integrate/auth/role/list/1",
          method: "get"
        })
        .then(res => {
          if (res.data.code === 0) {
    ...
    setDefaultRole() {
      this.roleNameList.forEach(item => {
        if (
          ...
          this.editForm.userRoles = [item.roleId];
        }
      });
    }
  },
